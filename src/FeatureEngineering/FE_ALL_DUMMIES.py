...
pad_hijo = pd.get_dummies(dataset["PAD_HIJO"])
hijo = pd.get_dummies(dataset["HIJO"])
padre = pd.get_dummies(dataset["PADRE"])
...
dataset = pd.concat([dataset,interacciones,oil_gas,campo,estado,lineamiento,pad_hijo,hijo,padre],axis=1)
